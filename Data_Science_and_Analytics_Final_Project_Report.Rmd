...
\n
Our independent column is "term_deposit". We will try to predict this variable with a high accuracy by using logistic regression on 2 datasets. One of these datasets won't have the rows that include NA values, other dataset will have these rows as their  NA values imputed. After predicting, we will compare the results.
\n
...
model_1 <- glm(term_deposit~.,data = train_data,family = "binomial")

prob <- predict(model_1 , newdata = test_data)
pred <- ifelse(prob>0.5 , 1,0)

conf_1 <- as.table(confusionMatrix(test_data$term_deposit,as.factor(pred)))
Sensitivity <- sensitivity(conf_1)
Specificity <- specificity(conf_1)
NegativePred <-  negPredValue(conf_1)
PositivePred <- posPredValue(conf_1)
Accuracy <- (conf_1[1,1] + conf_1[2,2])/(conf_1[1,1] + conf_1[1,2]+conf_1[2,1] + conf_1[2,2])
scores_1 <- data.frame(Accuracy,Sensitivity,Specificity,PositivePred,NegativePred)
...
model_2 <- glm(term_deposit~.,data = train_data,family = "binomial")

prob <- predict(model_2 , newdata = test_data)
pred <- ifelse(prob>0.5 , 1,0)
...
ggplot(real_value_vs_predicted_value, aes(x=value_name, y=Freq, fill=value_name)) +
  ...
  labs(
    ...
    y = "Counts",
    title = "Predicted Value Statistics")
conf_2 <- as.table(confusionMatrix(test_data$term_deposit,as.factor(pred)))
Sensitivity <- sensitivity(conf_2)
Specificity <- specificity(conf_2)
NegativePred <-  negPredValue(conf_2)
PositivePred <- posPredValue(conf_2)
Accuracy <- (conf_2[1,1] + conf_2[2,2])/(conf_2[1,1] + conf_2[1,2]+conf_2[2,1] + conf_2[2,2])
scores_2 <- data.frame(Accuracy,Sensitivity,Specificity,PositivePred,NegativePred)
scores <- cbind(t(scores_1),t(scores_2))
colnames(scores) <- c("Scores 1","Scores 2")
kable(scores,format = "markdown")
```
...
```
\n
When we look at the score tables, we can say that all of the scores ("accuracy", "sensitivity", "specificity", "positive prediction value", "negative prediction value") have gone up a little when we used imputed dataset as the input of logistic regression algorithm.
\n
\n
...
```
\n
The output dendrograms are really complicated, but it's obvious that best separation option is 2 clusters. When we try to use all of the data, program crushes. Therefore we used random sampling to decrease the number of rows in order to speed up the process of forming the model and preserve the randomness at the same time.


\n
We know that using 2 clusters is the correct answer, so we will choose kMeans clustering without pca. Also, we have a large number of variables and few number of clusters. Therefore, it's better to use kMeans clustering in that sense as well.
\n
\n
...
\n
Our independent column is "term_deposit". We will try to predict this variable with high accuracy by using "decision trees" and "kNN" algorithms on 2 datasets. One of these datasets will be balanced, other dataset will be imbalanced. After predicting, we will compare the results.
\n
...
conf <- as.table(confusionMatrix(as.factor(pred),test_data$term_deposit))
Sensitivity <- sensitivity(conf)
Specificity <- specificity(conf)
NegativePred <-  negPredValue(conf)
PositivePred <- posPredValue(conf)
Accuracy <- (conf[1,1] + conf[2,2])/(conf[1,1] + conf[1,2]+conf[2,1] + conf[2,2])
scores <- data.frame(Accuracy,Sensitivity,Specificity,PositivePred,NegativePred)
kable(scores,format = "markdown")
```
...
```
\n
When we used imbalanced data as an input of kNN with k=3, our accuracy was 89,3%. However, specificity is too low, that means the model is underfitted.Therefore, it's not healthy to use this model.
\n
```{r knn_balanced}
partial_data <- createDataPartition(y=perfect_data$term_deposit,p=0.8,list = FALSE)
train_data <- perfect_data[partial_data,]
...
```
\n
When we used balanced data as an input of kNN with k=3, our accuracy was 91,8%. Furthermore, other scores ("sensitivity", "specificity", "positive prediction value", "negative prediction value") were also much higher than the accuracy we achieved when we used imbalanced data as an input of kNN with k=3.
\n
\n

\n
When we look at their performance scores, balanced dataset gives us much higher scores when compared to imbalanced dataset in both algorithms. 

\n
Overall, if we compare algorithms with each other both in terms of speed and score, we can say that using balanced dataset with kNN algorithm is the best option for us. 
\n
